...
//page that lists all daytrack to be deleted
const Viewanddeletedaytrack = () => {
  ...
  return (
    <Container>
     <Typography variant="h3" gutterBottom sx={{ textAlign: 'center', marginBottom: '1rem',fontFamily: "Avenir Next" }}>
        Select to Delete DayTrack 
      </Typography>

      {dayTracks.length > 0 ? (
        <List>
          {dayTracks.map(dayTrack => (
            <ListItem key={dayTrack._id} component={Link} to={`/deletedaytrack/deleted/${dayTrack._id}`}>
...
export default Viewanddeletedaytrack;
